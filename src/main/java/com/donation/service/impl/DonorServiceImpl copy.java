...
// user.setPhoneNumber(donorDetails.getUser().getPhoneNumber()); // Updating
// phone number via User
// user.setAddress(donorDetails.getUser().getAddress()); // Updating address via
// User

// // Optionally update the donor's user association if needed (e.g., if
// changing
// // users)
...
// Campaign campaign =
// campaignRepository.findById(donationRequest.getCampaignId())
// .orElseThrow(
// () -> new RuntimeException("Campaign not found with id: " +
...
// donation.setDonationDate(LocalDate.now()); // Automatically set to current
// date
